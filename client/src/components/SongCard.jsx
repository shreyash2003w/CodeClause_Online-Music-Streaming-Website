...
const SongCard = ({ data, index, type }) => {
  ...
  const [
    { allArtists, allAlbums, allSongs, alertType, songIndex, isSongPlaying },
    dispatch,
  ] = useStateValue();
      ...
      {isDelete && (
        ...
        >
          <p className="text-xl text-headingColor font-semibold text-center">
            Are you sure to ,do you want to delete it ?
          </p>
          <div className="flex items-center gap-4">
            <motion.button
              className="px-2 py-1 text-xsm uppercase bg-red-300 rounded-md hover:bg-red-500 cursor-pointer"
              whileTap={{ scale: 0.7 }}
            ...
            <motion.button
              className="px-2 py-1 text-xsm uppercase  bg-green-300 rounded-md hover:bg-green-500 cursor-pointer"
              whileTap={{ scale: 0.7 }}
