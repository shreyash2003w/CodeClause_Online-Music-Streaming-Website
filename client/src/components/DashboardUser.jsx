...
export const DashboardUserCard = ({ data, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="w-275 min-w-[160px] text-center flex items-center gap-6 relative">
        ...
        {isUserRoleUpdated && (
          ...
          >
            <p className="text-textColor text-[12px] font-semibold ">
              Are you sure,do you want to mark user as{" "}
              <span>{data.role === "admin" ? "member" : "admin"} ?</span>
            </p>
            <div className="flex items-center gap-4 ">
              <motion.button
                whileTap={{ scale: 0.75 }}
                className="outline-none border-none text-sm px-4 pyl rounded-md bg-blue-200 text-black hover:shadow-md"
                onClick={() =>
                  updateUserRole(
                    data._id,
                    data.role === "admin" ? "member" : "admin"
              ...
              <motion.button
                whileTap={{ scale: 0.75 }}
                className="outline-none border-none text-sm px-4 pyl rounded-md bg-red-200 text-black hover:shadow-md"
